...
**Commands below are written bare** (case 1). Otherwise prepend the targeting you resolved:
`XORQ_DEFAULT_CATALOG=<name>` / `-n <name>` for a named catalog, or `-p ./<name>` for a repo-local one.
