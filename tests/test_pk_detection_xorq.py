...
def test_xorq_matches_polars_clear_single_pk():
    ...
    import polars as pl
    from pk_detection import detect_pk as detect_pk_polars

    data = {"row_id": list(range(N))}
    data.update(_filler_cols(N))
    data["extra"] = [i % 7 for i in range(N)]

    polars_result = detect_pk_polars(pl.DataFrame(data))
    xorq_result = detect_pk(ibis.memtable(data))
    assert polars_result == xorq_result


def test_xorq_matches_polars_no_pk():
    import random
    import polars as pl
    from pk_detection import detect_pk as detect_pk_polars
